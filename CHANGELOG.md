...
### Fixed
- **Session close no longer seals an empty receipt when the event log has one bad byte (Layer 0 audit).** `read_all_with_stats` aborted the *entire* read on the first non-UTF-8 byte (a partial write or corruption), and `session close` mapped that to `(no events, 0 skipped)` — sealing a receipt with an empty timeline and a zero skip-count exactly when the log was most damaged, silently bypassing the completeness signal. A bad line is now skipped and counted like a malformed JSON line, so the good events survive and the skip is surfaced. Regression-tested with a raw non-UTF-8 line.
- **`package verify` no longer implies it authenticated the session narrative.** The Merkle root in a `.treeship` package covers only the artifact IDs; the receipt's `timeline`, `side_effects`, `tool_usage`, and `narrative` are composed from the unsigned event log and are bound by no signature in the package — so editing them and re-serializing passed every check. `package verify` now carries an explicit `receipt_body_binding` line stating that only the artifacts and Merkle root are cryptographically bound, and that the authenticated record is the actor-signed `session.v1` (or the published report). This closes the overclaim; a follow-up will embed and require the `session.v1` signature in the package for full self-authentication.
### Fixed
- **Ed25519 verification is now strict across the whole core (Layer 0 audit).** Every signature-verifying path in `treeship-core` used the malleable, cofactored `verify` while `present.rs` alone used `verify_strict` — that inconsistency enabled signature malleability (a second byte-distinct envelope that still verifies and hashes to the same `artifact_id`) and cross-SDK split-view (one surface accepting what another rejects). All six core sites (the DSSE envelope verifier, checkpoint, agent certificate, participant join/countersign, hub-checkpoint, and invitation) now use `verify_strict`, which also rejects small-order public keys. Regression-tested; every genuine signature still verifies.
- **`treeship verify` now cryptographically checks chain linkage instead of asserting it.** The chain walk follows the artifact's *unsigned* `parent_id` storage metadata, which a local attacker can edit to truncate or rearrange a chain of individually-valid artifacts — yet `verify --full` printed `chain integrity: no tampering detected` regardless, a check it never ran. Verification now cross-checks each child's **signed** `parentId` (inside its re-verified envelope) against the walked parent, reports `signed linkage verified` only when it holds, and exits non-zero with `SIGNED LINKAGE BROKEN` on a mismatch (short, `--full`, and `--format json` all honor it). Verified live: an honest `--parent` chain verifies green; editing the stored parent to skip a link is caught and exits 1.
### Fixed
- **`verify-capability`: three ways it could be fooled, all closed (Layers 2-3 audit).** A deep audit of the capability layer found the cross-check could be defeated by the very party it audits, and that it trusted unverified signatures. Fixed: **(1)** the scope cross-check no longer lets `meta.tool` rescue a concrete out-of-scope action — a dishonest agent attaching `--meta '{"tool":"file.read"}'` to a `payments.charge` action no longer has it counted in-scope; the action label is authoritative and `meta.tool` contributes scope only when the label is a generic dispatch placeholder (`tool.call`/`tool.use`/…, the legitimate MCP case). **(2)** Key-boundness, the action cross-check, and revocation authorization now use the **re-verified** signer (from `verify_any` against your trust roots), not the unverified `signatures[0].keyid` — a card or revocation carrying a forged first signature naming a victim's key can no longer report key-bound, inflate the in-scope count, or revoke a card it does not control. **(3)** Provenance grades (`captured`/`discovered`) read off a card that is **not key-bound** are now marked `SELF-ASSERTED (not machine-verified)` in the output, since a self-asserted card can carry fabricated grades that would otherwise print identically to real ones; the published `agent_card.v1` schema now also constrains `grade` to the four defined values. Legitimate flows (real captured cards, real self-revocations) are unaffected; verified against live exploit reproductions.
### Added
