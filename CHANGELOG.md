...
- **Trust documentation rewritten exhaustively.** The previous `TREESHIP.md` / `CLAUDE.md` trust block claimed `@treeship/mcp` captured "arguments passed to the tool". The bridge actually captures the **SHA-256 digest** of arguments and output, never raw values. The first-pass fix narrowed that wording but still omitted other emitted fields (`action`, `approval_nonce`, `server`, `agent_name`, `artifact_id`, `meta.source`, etc.). The trust block is now organized by attestation type (intent attestation / result receipt / session event) and **enumerates every field the bridge actually emits**, verified line-by-line against `bridges/mcp/src/client.ts`. The block also flags that `payload.error_message` carries the raw `Error.message` string on thrown errors (so failures stay debuggable — treat it like a logged stack trace if your tools can leak in error text).
...
- **Setup script version gate uses semver-correct comparison.** Replaced `sort -V` (which incorrectly satisfies `>= 0.9.3` for prereleases like `0.9.3-rc.1`) with a node-based parser that treats any prerelease of `0.9.3` as `< 0.9.3`. Smoke-tested against `0.9.2`, `0.9.3-rc.1`, `0.9.3`, `0.9.3-alpha`, `0.9.4`, `0.10.0`, and `1.0.0`. Upgrade hint also softened: no more `rm $(command -v treeship)` — points at package-manager reinstall instead.
