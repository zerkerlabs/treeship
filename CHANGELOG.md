...
- **Trust documentation corrected.** The previous TREESHIP.md / CLAUDE.md trust block claimed `@treeship/mcp` captured "arguments passed to the tool". The bridge actually captures only the **SHA-256 digest** of arguments and output (see `bridges/mcp/src/client.ts`); raw argument values, raw output content, and file contents are never stored. Error message text **is** captured raw (so failures stay debuggable) — flagged in the trust docs so users treat it like a logged stack trace.
- **Plugin `post-tool-use.sh` no longer regex-extracts `tool_name`** from the PostToolUse JSON payload (a greedy `sed` would pull the wrong value if `tool_input` itself contained the substring `"tool_name":"foo"`). It now uses `jq` when present and falls back to recording `unknown` rather than risk wrong-tool attribution.
- **Setup script version gate.** `setup.sh` now warns loudly if the installed CLI is below v0.9.3 (the version that introduces the TREESHIP.md drop), so a user running the script before v0.9.3 publishes doesn't end up with the script's `read ./TREESHIP.md` hint pointing at a nonexistent file.
