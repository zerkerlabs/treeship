...
- **"Verify in the browser" says what it verifies.** From a receipt JSON alone, `@treeship/verify` checks structure (Merkle root, inclusion proofs, leaf count, timeline) and reports `structural-pass`; signatures and the issuer need the sealed package. The SDK overview, the Merkle proofs guide and the edge runtime guide now say so wherever they said "verify in the browser" (TASKS-0.31.6 T18).
