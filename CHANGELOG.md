...
- **Receipt verification no longer reports an unsigned receipt as authentic (AUD-01).** `treeship verify <package|url>` and the WASM `verifyReceipt` graded a Session Receipt on its internal Merkle consistency alone — recompute the root, check the inclusion proofs, done — a self-referential check that never verifies a signature against a trust root. A fabricated or tampered receipt (an empty one included) therefore passed as authentic. The verdict is now gated on a real authenticity check: a receipt that is internally consistent but carries no verified signature reports **`unverified`** — the CLI prints "authenticity is NOT established" and `--format json` reports outcome `unverified`, not a green pass. A new `--strict` flag fails closed (exit 1) for automation. `@treeship/verify` and the TypeScript SDK add `unverified` to `VerifyReceiptResult.outcome`. Structural consistency is not authenticity.
- **The Hub DPoP signing key is now sealed at rest (AUD-02).** `treeship hub attach` wrote the dock's DPoP private key to `config.json` as raw hex — any process, backup, or file sync that could read it recovered a usable signing key and could mint DPoP proofs for the connection. It is now sealed with the same AES-256-GCM keystore AEAD that protects ship keys, with the hub id bound into the AAD so a sealed key cannot be transplanted between connections. Legacy plaintext values still load for one migration cycle and are re-sealed on the next write.
