...
- **Anonymous usage telemetry, opt-out, with the numbers published.** Treeship is local-first, which left the project unable to tell how many machines run it: registry download counts are mirror and CI traffic, and the hub only sees docks that attach. The CLI now sends one ping on its first run on a machine and at most one a week after that: a random install id (16 bytes, generated locally, not a key and not the ship id), the CLI version, OS, architecture and harness (`claude-code`, `codex`, `cursor` or `unknown`). Nothing else, ever: no paths, hostnames, receipts, command lines or IP (the hub does not store the request address). Off with `treeship telemetry disable`, `DO_NOT_TRACK=1` or `TREESHIP_NO_TELEMETRY=1`, and never sent when `CI` is set; `treeship telemetry status` prints the exact payload; `init` says so. The hub accepts only that closed field set (`POST /v1/telemetry`, allowlisted values, 2 KiB cap, one row a day per id), keeps raw events for 90 days and per-day rollups and per-install first/last seen forever, and `/v1/stats` gains an `installs` block: total, new and active over 7 and 30 days, by version, harness and OS, with the same kind of basis caveat the agents block carries. Builds from source send nothing unless `TREESHIP_TELEMETRY_ENDPOINT` is set, so `cargo test` never counts. Page: [Telemetry](docs/content/docs/about/telemetry.mdx).
