...
> **Update, 7 September 2026.** The "next piece" below shipped in 0.29.0: the merchant's approval is now a signed, single-use grant, and the apply receipt spends it once. The details, and the replay refusal, are in [The operator's yes, signed once](/blog/merchant-approvals-signed-once); the wiring is in the [guide's approvals section](/commerce/commerce-agents#approvals-the-operators-yes-spendable-once). The test count below is as of publication; it is twenty-eight now.
