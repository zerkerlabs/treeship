...
> This post reads a published open standard and describes an open-source integration with it. Treeship is not endorsed by, coordinated with, or speaking for Mastercard or the standard's maintainers. Since 0.31 the integration is `treeship vi`, an independent implementation of the v0.1 draft tested against the reference SDK.
