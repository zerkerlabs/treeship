...
There is a comfortable story about agent risk. The dangerous capability lives in a handful of frontier labs, so the controls live there too: slow the research, gate the front door, and the rest of us are covered.
...
**Local-first.** The record is produced on the machine doing the work, by the process doing the work, before anything leaves. There is no central service in the critical path, because a central service is a front door, and the whole point is that front doors do not cover the network.
...
The bet is simple. Autonomy is going to be granted to agents at the rate we can hold them accountable, and no more. If accountability stays at the front door, autonomy stays shallow, or it goes deep without accountability, which is the outcome everyone says they want to avoid.

The alternative is a substrate that spreads the way the agents spread: installed locally, signed by the actor, chained, portable, and verifiable by anyone who holds the bytes and a public key. That is what we are building, and it is why the verifier is open, the format is documented, and nothing about it depends on asking us.
