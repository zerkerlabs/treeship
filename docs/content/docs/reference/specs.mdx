...
| [commitments](https://github.com/zerkerlabs/treeship/blob/main/docs/specs/commitments.md) | Draft | Proving what an agent *promised* vs what it did. Slices 1-4 unbuilt; two adjacent ideas shipped separately (`AuthorityUse` #297, objective binding #298) |
| [time-anchoring](https://github.com/zerkerlabs/treeship/blob/main/docs/specs/time-anchoring.md) | Draft, not implemented | Receipt timestamps are self-asserted (`SystemTime::now()`, signed by the agent's own key). Specifies external witnessing so *when* is provable, not just *what* |
...
| [workflow-declarations](https://github.com/zerkerlabs/treeship/blob/main/docs/specs/workflow-declarations.md) | Slices 1-2 + slice-3 primitives | Golden reports, pure reducer, registered/signed `workflow.v1`, real checkpoint pre-existence verification, and signed first-run binding via `session start --workflow-ref`. Automatic checkpoint composition and CLI conformance verification remain |
...
| [agent-to-agent-verification](https://github.com/zerkerlabs/treeship/blob/main/docs/specs/agent-to-agent-verification.md) | Draft; protocol ships, enforcement does not | `present` / `verify-presentation --challenge`, `agent_cert.v1` chains, and trust-root pinning all ship (beta). What is unbuilt is the default-on gate: bridges and harnesses still execute inbound foreign work without requiring the handshake to pass first |
| [grok-bot-a2a](https://github.com/zerkerlabs/treeship/blob/main/docs/specs/grok-bot-a2a.md) | Draft, not implemented | Implementation spec for the first chat-native host. Runtime facts verified against docs.x.ai 2026-09-01; no code has run on a Grok Bot VM |
| [grok-bot-h2a](https://github.com/zerkerlabs/treeship/blob/main/docs/specs/grok-bot-h2a.md) | Draft, not implemented | Harness note under agent-to-agent-verification: the two human verbs ("prove you", "receipt this") expressed as the same handshake with a person minting the nonce |
