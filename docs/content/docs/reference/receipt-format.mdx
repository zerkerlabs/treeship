...
Take the first 16 bytes (128 bits) of the 32-byte SHA-256 digest, *then*
hex-encode -- not the other way around. `hex(digest)[..16]` truncates the
*hex string* to 16 characters (8 bytes of entropy, and the wrong id
entirely); `hex(digest[..16])` truncates the *digest* to 16 bytes first,
giving the correct 32-character hex id. Same content always yields the same
id.
