...
| Retries as a signed chain (attest action --retry-of, verify row retries) | `beta` | `treeship attest action --retry-of`, `treeship package verify` | [attest](/docs/cli/attest), [package](/docs/cli/package) |
| Workload packet and recomputation receipts | `beta` | `treeship attest receipt` | [Predicate registry](/docs/reference/predicates), [Verification reporting](/docs/guides/verification-reporting) |
| Evaluation receipts | `beta` | `treeship attest receipt` | [Predicate registry](/docs/reference/predicates) |
| Judgement receipts (judgement.v1, judgement.resolution.v1) | `beta` | `treeship attest receipt`, `treeship package verify` | [Predicate registry](/docs/reference/predicates), [package](/docs/cli/package), [judge](/docs/cli/judge) |
| Judge slot (treeship judge) | `beta` | `treeship judge` | [judge](/docs/cli/judge), [Claude Code](/docs/integrations/claude-code), [TypeSafe Jev](/docs/integrations/typesafe-jev), [Predicate registry](/docs/reference/predicates) |
| Kill switch (halt.v1) | `beta` | `treeship halt` | [Predicate registry](/docs/reference/predicates), [Claude Code](/docs/integrations/claude-code), [@treeship/mcp](/docs/integrations/mcp), [@treeship/a2a](/docs/integrations/a2a) |
...
| Per-artifact Rekor anchoring | `stable` | `treeship hub push`, `treeship verify --max-unwitnessed`, `treeship package verify` | [TS-2026-003 spec](https://github.com/zerkerlabs/treeship/blob/main/docs/security/TS-2026-003.md), [time-anchoring spec](https://github.com/zerkerlabs/treeship/blob/main/docs/specs/time-anchoring.md) |
| Checkpoint-root anchoring | `roadmap` | -- | -- |
| RFC 3161 trusted timestamps | `roadmap` | -- | -- |
| OpenTimestamps anchoring | `roadmap` | -- | -- |
...
| Workflow declarations and conformance | `experimental` | `treeship attest receipt`, `treeship session start`, `treeship workflow verify` | [workflow-declarations spec](https://github.com/zerkerlabs/treeship/blob/main/docs/specs/workflow-declarations.md) |

## Identity, approval, attribution

| Feature | Status | CLI | Docs |
|---|---|---|---|
| Approval Use Journal | `stable` | `treeship approval uses`, `treeship approval status`, `treeship approval journal verify` | [Approval Use Journal](/docs/concepts/approval-use-journal), [Approval Authority](/docs/concepts/approval-authority), [approval](/docs/cli/approval) |
...
| Claude Commerce Agents receipts (treeship-commerce) | `beta` | -- | [Claude Commerce Agents](/docs/integrations/commerce-agents) |
...
| Feature | Status | CLI | Docs |
|---|---|---|---|
| Verifiable Intent (treeship vi) | `beta` | `treeship vi keygen`, `treeship vi check`, `treeship vi attest`, `treeship vi verify` | [treeship vi](/docs/cli/vi), [Verifiable Intent](/docs/integrations/verifiable-intent) |
...
| Feature | Status | CLI | Docs |
|---|---|---|---|
| ZK -- Circom Groth16 (snarkjs shell-out) | `quarantined` | `treeship prove`, `treeship verify-proof`, `treeship zk-setup` | [Zero-knowledge proofs](/docs/concepts/zero-knowledge), [prove / verify-proof / zk-setup](/docs/cli/zk) |
| ZK -- RISC Zero local prover (prove-chain) | `experimental` | `treeship prove-chain` | [Zero-knowledge proofs](/docs/concepts/zero-knowledge), [prove / verify-proof / zk-setup](/docs/cli/zk) |
| ZK -- Circom native pure-Rust (ark-circom) | `roadmap` | -- | -- |
| ZK -- RISC Zero Bonsai (hosted proving) | `roadmap` | -- | -- |
| ZK -- TLSNotary (verifiable TLS evidence) | `roadmap` | `treeship zk-tls-setup` | [prove / verify-proof / zk-setup](/docs/cli/zk) |
