...
[`evaluation.v1`](#evaluationv1) can point at it through its `coverage`
...
any that was acted on against its own bar (a yes answer allowed, a no answer refused, or no bar declared). [`treeship judge`](/docs/cli/judge)
mints these: the built-in rules judge, or any judge behind `--judge-url`.

| Field | Type | Required |
|---|---|---|
| `schema` | const `judgement.v1` | yes |
| `judge` | object `{model, provider, kind, replayable}` with `kind` one of `decision-model`, `llm`, `classifier`, `rules` | yes (`model`) |
...
[`treeship judge --resolve`](/docs/cli/judge#escalation-and-the-humans-decision)
