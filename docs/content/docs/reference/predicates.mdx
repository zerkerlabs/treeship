...
any that was acted on below its own threshold.

| Field | Type | Required |
|---|---|---|
| `schema` | const `judgement.v1` | yes |
| `judge` | object `{model, provider, kind: decision-model|llm|classifier|rules, replayable}` | yes (`model`) |
| `state_digest` | string (`sha256:<hex>`) | no |
| `questions_digest` | string (`sha256:<hex>`) | no |
| `question` | object `{key, type: noul|choice|score, instructions, options[]}` | yes (`key`, `type`) |
| `answer` | object `{noul, choice, score, probabilities{}, confidence}` | yes |
| `threshold` | object `{value, applies_to: confidence|noul, set_by}` | no |
