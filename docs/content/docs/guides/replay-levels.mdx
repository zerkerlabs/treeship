...
| `local-journal` | The recorded use is within `max_uses` per the **ship's local journal**, and the use record names this action | Says nothing about other machines. The journal lives beside the keystore, so every config that shares a ship (a project stub extending the global config) shares it; a workspace with its own keys has its own | Verifier has Treeship workspace |
