...
`curl -fsSL https://www.treeship.dev/install | sh` tries the prebuilt binary for your
...
The current release publishes four binaries: `treeship-darwin-aarch64`,
`treeship-darwin-x86_64`, `treeship-linux-x86_64` and `treeship-linux-aarch64`.
Linux arm64 has shipped since v0.25.1 -- `@treeship/cli-linux-arm64` is a real
`optionalDependencies` entry the npm wrapper resolves, not a cargo-build
fallback. Both `curl | sh` and `npm install -g treeship` install the prebuilt
binary directly on all four platforms today.
