...
All official templates ship inside the CLI binary. No network access required.
...
| `robinhood-agentic-trading` | Conservative receipts and approval posture for agents connected to Robinhood Trading MCP. |
...
session:
  ...
  auto_push: false # not implemented; parsed but has no effect
...
hub:
  auto_push: false # not implemented; parsed but has no effect — push explicitly with `hub push` or `session report`
...
| `auto_push` | bool | false | **Not implemented.** Parsed but has no effect; push explicitly with `hub push` or `session report` |
