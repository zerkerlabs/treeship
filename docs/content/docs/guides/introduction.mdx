...
## Architecture: agents, Treeships, and hub connections
...
**Layer 2: Treeships** -- the trust domain itself. One Treeship equals one Ed25519 keypair and one local artifact store. A Treeship is your identity boundary. All artifacts signed by a Treeship share the same root of trust.

**Layer 3: Hub connections** -- how a Treeship connects to the outside world. A hub connection is a named link from your Treeship to a Hub workspace. Think of hub connections like tmux sessions -- named connections you can attach, detach, and kill independently.

Most developers need exactly one hub connection. You would create multiple connections if you publish artifacts to separate audiences -- for example, one connection for your personal workspace and another for a client's workspace. The Treeship key stays the same across all connections, and your artifacts never change. Hub connections only control where artifacts appear on Hub.
