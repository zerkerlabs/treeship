...
The receipt page at treeship.dev runs the same structural checks client-side via WASM, from the same Rust code. From a receipt JSON alone that is the Merkle root, the inclusion proofs, the leaf count and the timeline; envelope signatures and the issuer are not verified, because the receipt does not carry the signed bytes. `treeship package verify` on the sealed package is the check that does.
