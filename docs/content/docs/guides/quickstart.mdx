...
```bash
# Connect your ship to Hub (creates your default hub connection)
treeship hub attach
# Visit https://treeship.dev/hub/activate
# Type the code, click Approve

# Push the artifact to your active hub connection
treeship hub push $TREESHIP_LAST
...
<Callout type="info">
Most developers only need one hub connection. If you work across multiple organizations, you can create named connections with `treeship hub attach --name acme-corp` and switch between them with `treeship hub use acme-corp`. See the [trust model](/docs/concepts/trust-model) for details.
</Callout>
</Step>
</Steps>
