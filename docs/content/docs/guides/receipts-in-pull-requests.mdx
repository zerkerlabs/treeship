...
`git interpret-trailers` and `git log --format=%(trailers:key=Treeship-Receipt)` both read it. The digest is `sha256(receipt.json)`, which is what `package verify` recomputes and what the close record signs, so a rewritten package and a stale trailer disagree in a way the check can see.
