...
This endpoint has two calling modes depending on whether keys are provided. The browser calls it after the user approves the device code. The CLI calls it with keys to complete the hub connection creation.

## Browser-only call (no keys)

When the browser confirms approval without providing ship/hub keys, Hub marks the challenge as approved and returns a status. No hub connection or ship record is created yet.
...
When the CLI calls this endpoint with ship and hub public keys, Hub verifies the challenge is already browser-approved, validates the nonce, atomically consumes the challenge, creates the ship record (if new), and returns a hub_id.
...
- The ship public key and hub public key must be different Ed25519 keys
...
- If the `ship_public_key` already exists on Hub, the existing ship record is reused and a new hub connection is attached to it
