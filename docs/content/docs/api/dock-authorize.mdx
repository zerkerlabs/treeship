...
When the CLI calls this endpoint with Treeship and dock public keys, Hub verifies the challenge is already browser-approved, validates the nonce, atomically claims the challenge for a new `dock_id`, creates the Treeship record, and returns it. The claim is single-use: a concurrent or replayed finalize is rejected as `already_attached`.
...
{
  "state": "attached",
  "dock_id": "dck_9f8e7d6c5b4a39281706f5e4d3c2b1a0",
  "next_steps": [
    "treeship status",
...
| `dock_id` | string | Unique identifier for this hub connection (format: `dck_` + 32 hex chars) |
...
- The Treeship public key and dock public key must be different Ed25519 keys
