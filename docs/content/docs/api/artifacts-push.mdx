...
This endpoint requires DPoP authentication. The `Authorization` header must be `DPoP {dock_id}`, and the `DPoP` header must be a valid JWT signed by the dock private key with:
...
| `401` | `{"error": "dock not found"}` | Unknown dock ID |
...
<Callout type="info">
In practice, you do not call this endpoint directly. The CLI handles artifact construction, signing, and pushing when you run `treeship dock push`.
</Callout>
