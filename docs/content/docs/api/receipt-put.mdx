...
Uploads a [Session Receipt v1](/docs/concepts/session-receipts) JSON document to the hub and makes it available at the permanent public URL `https://treeship.dev/receipt/{session_id}`.
...
The request body is the canonical `receipt.json` from the `.treeship` package,
sent verbatim. This v1 endpoint stores the receipt JSON only. It does not upload
the full `.treeship` package, `package-manifest.json`, or side evidence.
...
PUT is write-once for a `session_id`. A byte-identical retry by the owning dock
is accepted as an idempotent replay. A different body for an already stored
`session_id` is rejected instead of overwriting the original receipt.
...
<Callout type="warn">
This endpoint is a share/index endpoint, not a package-verification endpoint. For
the strongest offline proof, distribute the `.treeship` package and run
`treeship package verify` so the signed package manifest and package-local
