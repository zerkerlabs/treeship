...
| `GET` | `/v1/version` | None | Which build is running: version, commit, build time, Go runtime (`/version` is an alias; `/health` and `/healthz` are the liveness probe, `/readyz` readiness) |
...
```json
{"service":"treeship-hub","version":"0.31.10","commit":"1a2b3c4","built_at":"2026-09-26T00:00:00Z","go":"go1.26.4"}
```

Values come from the build; unset ones read `dev` or `unknown` rather than a guess. Never cached. `/version` is an alias, `/health` and `/healthz` answer liveness, `/readyz` readiness.
