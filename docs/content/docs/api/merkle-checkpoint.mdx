...
Accepts an optional `dock_id` query parameter to scope the result to a specific dock.

```
GET /v1/merkle/checkpoint/latest?dock_id=dck_abc123
```
...
| `dock_id` | string or null | Dock that published this checkpoint |
...
# Get the latest checkpoint for a specific dock
curl "https://api.treeship.dev/v1/merkle/checkpoint/latest?dock_id=dck_abc123"
```
