...
| `dock` | `DockModule` | Hub push/pull and connection status |
...
Check the current dock connection status.

```ts
const status = await s.dock.status();
if (status.docked) {
...
| Field | Type | Description |
|-------|------|-------------|
| `docked` | `boolean` | Whether the dock is currently connected |
| `endpoint` | `string \| undefined` | Hub endpoint URL, if connected |
| `dockId` | `string \| undefined` | Dock identifier, if connected |

If the status check fails for any reason (CLI not installed, no config), returns `{ docked: false }` instead of throwing.
