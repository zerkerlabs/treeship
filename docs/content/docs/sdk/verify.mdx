...
<Callout type="info">
**Node 22 works.** `@treeship/core-wasm@0.24.0` shipped only a wasm-pack
`--target bundler` build, which doesn't load in Node at all
(`WebAssembly.Table.grow(): failed to grow table by 4`) -- every export was
affected. Fixed in v0.25.0: the build produces both a bundler target and a
separate Node.js (CommonJS) target, and the package's `exports` map routes
`node` to it. Tested here: `@treeship/verify@0.31.9` loads and runs
`verifyReceipt` on Node 22.23.1.
</Callout>
...
| URL string | `'https://treeship.dev/receipt/ssn_abc'` | Fetched with global `fetch`; `/receipt/<id>` maps to `/v1/receipt/<id>`, an API URL (`/v1/receipt/<id>`) is fetched as written, anything that names no receipt throws |
