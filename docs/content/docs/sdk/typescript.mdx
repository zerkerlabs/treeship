...
line**, not a bare semver — e.g. `"treeship 0.31.9 (rust)"`. Parse it if you
need the number; do not compare it to a fixed string.

```ts
import { Ship } from '@treeship/sdk';

const line = await Ship.checkCli();      // "treeship 0.31.9 (rust)"
const version = line.split(' ')[1];      // "0.31.9"
...
Note: The CLI's `attest decision` accepts `--model-version`, but the TypeScript SDK's `modelVersion` field is not yet wired to it -- setting it has no effect. Use the `model` field to identify the model and version together (e.g., `"claude-opus-4-20260301"`) until the SDK forwards it.
...
These five load `@treeship/core-wasm`; `s.verify.verify()` shells out to the
CLI instead. See the [`@treeship/verify` page](/docs/sdk/verify#runtime-support)
for the WASM package's Node/browser support.
