...
The owner holds the private keys, encrypted at rest. Keys never leave the local machine unless explicitly exported (and `keys export` prints only the public half). There is no key escrow or recovery service.

**Rotation is local.** `keys rotate` mints a successor in this keystore and stamps the predecessor with a grace-period end, and that is all it does. Nothing is minted that a counterparty could verify, nothing travels in a bundle or a presentation, and a pinned trust root has no expiry field. A counterparty who pinned the predecessor keeps trusting it until they remove the pin, and cannot verify anything the successor signs until they pin it. Re-pinning is a manual, out-of-band step: send them the lines `keys export` prints for the successor, and ask them to `trust remove` the predecessor. If the predecessor was compromised, assume every counterparty trusts it until you hear back. There is no succession artifact yet; see [keys](/docs/cli/keys#keys-list--keys-rotate).
...
| `transparency_log` | Accept this Rekor log's signed entry timestamps as witnessed time for per-artifact anchors (`package verify`'s `anchoring` row, `verify --max-unwitnessed`). Does not cover Merkle checkpoints -- those are not anchored to Rekor. ECDSA P-256, given as `@rekor.pub` or `ecdsa-p256:<base64url DER>`. Sigstore's public-good log is built in; any pin of this kind replaces it |
