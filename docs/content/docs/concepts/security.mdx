...
- Private key encrypted at rest (AES-256-GCM) under a key derived from a secret seed file stored alongside the keystore -- not from a passphrase; nothing you type or remember protects it
- Key never leaves the local machine unless explicitly exported
- No key escrow, no key recovery service -- losing the seed file makes the keystore unrecoverable

{/* claims:keystore-at-rest */}
<Callout type="warn">
Root access to the machine breaks all guarantees. The trust boundary is the machine itself: anyone who can read your `.treeship` directory (keys plus the seed file beside them) can copy it and sign as you. There is no passphrase to additionally compromise.
</Callout>
...
{/* claims:keystore-at-rest */}
- A compromised signing key (anyone who reads your `.treeship` directory -- the key plus the seed file that decrypts it -- can forge artifacts; there is no passphrase gating this)
...
Grant revocation is a signed receipt, not a Hub verdict. `treeship grant revoke` mints a `grant_revocation.v1` artifact signed by the grantor. `treeship verify` honors it from the local store or from a Hub's published list, and `treeship grant show --check-log` re-verifies the dock's consistency chain with RFC 6962.

The Hub serves the revocations it has received at:

```
GET /.well-known/treeship/revoked.json
```

Each entry carries the grantor-signed DSSE envelope plus `dock_id` and `rekor_index`, so a verifier checks the entry against the grantor's key and can request an inclusion proof for it. **The list itself is not signed by the Hub.** The Hub holds no signing key, and a Hub signature would only attest that the Hub said so — strictly less than the grantor's signature already on each entry. The document states what it cannot prove: absence from the list is not evidence a grant was never revoked, because a withheld entry looks identical to one that never existed. When the query fails the Hub answers 503, never an empty 200.

Offline verifiers see only the revocation artifacts they possess; a revocation minted after a receipt was packaged is invisible until a log is consulted. Unknown stays a third state throughout — `verify` reports authority as unverified rather than passing.
