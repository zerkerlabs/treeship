...
`--network api.example.com,*.internal.net` adds a network scope to the card as `capabilities.network`: the hosts the agent may reach, exact or by `*.suffix`. `verify-capability` reports it as `declared network`. The judgement happens where the evidence is: a session started with the same scope in its [declaration](/docs/concepts/agent-identity#network-scope) seals every recorded destination against it, and `package verify` reports the result as the `network_scope` row.
...
This mints a signed `agent_card_revocation.v1` receipt. verify-capability honors a revocation **only when its signer is authorized**: the card's own key (self-revocation) or a key pinned on the verifying machine under `--kind revoker`. The output names which: `self-revoked (the card's own key)` or `pinned revoker key_… (a key this machine pinned under --kind revoker)`. A pinned revoker is not necessarily the card's issuer; it is whichever ship the verifier chose to grant that power, which is why `keys export` prints the `revoker` pin as a separate line to hand over deliberately. An unauthorized revocation, signed by any other key, is ignored. When an authorized revocation is present the card's status becomes `REVOKED` and a "do not honor" warning is shown. A stranger cannot revoke your card.

## Verifying in the browser

The same check runs client-side. `@treeship/verify` exposes `verifyCapability(card, actions, trustRoots)`, backed by a WASM export, that returns the **same verdict the CLI does**, key-bound status, declared tools, and the in/out-of-scope cross-check. The matching logic lives in one shared Rust module (`treeship_core::capability`) used by both the CLI and the WASM verifier, so a browser receipt viewer and the command line cannot disagree.
