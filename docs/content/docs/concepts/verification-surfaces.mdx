...
| `treeship package verify` | Package/artifact integrity: determinism, Merkle root, inclusion, plus approval-evidence replay rows | Local keys + pinned roots | `verified`, with `receipt_body_binding` saying whether the close record binds the narrative (it does since 0.31.4) |
