...
```bash
treeship verify --zk artifact.json
```

Verify a full session proof:

```bash
treeship verify --session session_proof.json
```

Verify everything at once:

```bash
treeship verify --all session_proof.json
```
...
| RISC Zero proofs | Alpha | v0.6.0 |
| TLSNotary | Alpha | v0.7.0 |
