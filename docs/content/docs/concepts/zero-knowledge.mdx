...
<Callout type="warn">
**Current status (v0.17.0, re-audited 2026-07-06).** Unchanged since this
callout was first written at v0.10.4: only the first two layers ship enabled
by default, and no ZK development has occurred since — deliberately. The ZK
layers are maintained but not advanced; demand pulls them forward, not
roadmap pressure. Layers 3 and 4 remain behind the `--features zk` build
flag with additional runtime requirements:
