...
A counterparty pins the ship key once and can then verify **every agent that ship ever certifies**. `treeship keys export` prints the exact `trust add` lines, one per power (the v0.19 trust-kind split): `--kind cert_issuer` is the one the chain walk consults; add `--kind hub_checkpoint` for the staple/transparency verdict, and `--kind revoker` to honor the ship's capability revocations. Pin only the powers you intend to grant. Verdicts say how they were established explicitly:
...
```bash
# verifier: mint a fresh nonce (session mint-challenge records issued_at, so the
# window can be checked; openssl rand -hex 16 works too)
NONCE=$(treeship session mint-challenge --format json | jq -r .nonce)
