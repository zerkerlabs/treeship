...
that nothing is wrong. A violation is a failed verification: `treeship verify`
reports `outcome: fail`, counts it in `failed` and `authority_failed`, and exits
1 by default, flag or no flag.
