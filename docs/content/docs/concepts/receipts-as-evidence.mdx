...
| When? | The signed timestamp, which proves only that the key asserted that time; an external anchor (hub checkpoint, later Rekor or OpenTimestamps) brackets it | `signed` in the payload; anchoring coverage in `verify` |
