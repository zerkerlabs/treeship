...
<Callout type="warn">
**Design sketch, not built.** No checkpoint-root anchoring exists anywhere
today: the hub does not sign or countersign checkpoints, and there is no
OpenTimestamps or Bitcoin anchoring code in this repo. What *is* shipped
(0.31.9, TS-2026-003) is per-artifact Rekor anchoring -- a pushed artifact's
own dsse entry in Sigstore's transparency log, verified offline -- which is
a different, narrower thing than anchoring a room's checkpoint root. Read
this section as "the design would work this way," not "this is what
`treeship room` does."
</Callout>
...
5. Hub or a local worker will be able to anchor the checkpoint to OpenTimestamps, Bitcoin, or a transparency log.
...
Participants no longer have to be on the same machine: `session join --out`
writes the pending join to a file a joiner can send to the host, and
`session countersign --pending` finalizes it there, gated on a mandatory
liveness challenge (`session mint-challenge` / `answer-challenge`). See
[Joining from another machine](/docs/concepts/multi-agent-sessions#joining-from-another-machine)
for the full flow.
