...
| `hooks/hooks.json` | SessionStart (auto-start), SessionEnd (seal + report URL), PostToolUse (capture built-in tools, tagged with the subagent instance when a subagent made the call), SubagentStart / SubagentStop (`agent.spawned` and `agent.returned`, so the receipt's agent graph carries the spawn) |
