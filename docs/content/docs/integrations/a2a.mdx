...
if (!gate.allowed) return refuse(gate.refusal, gate.message); // halted | no_presentation | no_challenge | challenge_failed | untrusted_issuer | revoked | stale | verification_failed | gate_unavailable
...
`treeship halt <actor>` (or `*`) stops this middleware. `admitTask` asks `treeship halt list` before it looks at the presentation and, while a halt this ship signed stands against the middleware's actor, refuses with `halted`; `onTaskReceived` asks again, so a local task with no `fromAgent` is stopped too, and throws `TreeshipHaltedError`. Each refusal is signed as a `blocked.v1` receipt (`reason_class: operator_revocation`, naming the halt) chained onto the session, the same receipt the Claude Code gate mints. `treeship halt --lift <actor>` ends it.

A check that cannot run (no CLI on the PATH, a CLI older than 0.31.6) fails open, said once on stderr; under `TREESHIP_STRICT=1` it refuses. `TREESHIP_DISABLE=1` skips the check with everything else.
...
| `TREESHIP_STRICT=1` | A halt check that cannot run refuses the task instead of proceeding unchecked. |
| `TREESHIP_A2A_UNVERIFIED=1` | Runs foreign work whose gate would have refused, and signs `a2a.gate.skipped`. Never skips a halt. |

## Design rules

- Treeship errors **never** fail the underlying A2A handler. The two exceptions are decisions, not errors: the inbound gate and the kill switch.
