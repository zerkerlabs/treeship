...
<Callout type="info">
A config path under your home that is a symlink (a dotfiles checkout, or `/tmp` on macOS, which is `/private/tmp`) is followed: the Treeship entry lands where the link points and the link itself is kept.
</Callout>
