...
| **OpenClaw** | `treeship add openclaw` | `treeship add openclaw` | `cd integrations/openclaw-plugin && npm install && npm run build && openclaw plugins install --local ./` | 🟡 candidate |
...
| **Kimi Code CLI** | `npx skills add zerkerlabs/treeship --skill treeship --agent kimi-cli -g -y` | `kimi mcp add --transport stdio treeship --env TREESHIP_ACTOR=agent://kimi-code -- npx -y @treeship/mcp` | `see integrations/kimi-code-plugin/config.toml.template (no automated installer yet)` | 🟡 candidate |
