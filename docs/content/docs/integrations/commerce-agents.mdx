...
- **Charge a card.** `receipted_backend()` signs the cart and the hosted-URL digest at `checkout_handoff`, and `order_placed()` lets the host chain its order onto that. Payment itself stays with the host; the receipt says what was handed off and what was placed, never that money moved.
