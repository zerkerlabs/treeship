...
<Callout type="warn">
**The full remote attach flow is still not built.** You can register a SuperNinja card so it appears in your inventory, but `verified_captures` stays empty: as of v0.24 neither `treeship agent invite` nor `treeship join --invite` exists, so the harness cannot be enrolled inside the remote VM this way. This was slated for v0.9.9 and did not ship. Run `curl -fsSL https://www.treeship.dev/setup | sh` inside the VM and onboard there instead.
</Callout>
...
remote agent today, run `curl -fsSL https://www.treeship.dev/setup | sh` inside the VM and
