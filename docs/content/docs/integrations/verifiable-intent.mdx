...
<Callout type="info">
Shipped in 0.31 against the **v0.1 draft** of the specification. The implementation is complete for that draft and tested against its reference SDK in CI; the draft itself is not final, so the CLI names it as a draft and the claim shapes here may move when the standard does. The command reference is [`treeship vi`](/cli/vi). Before 0.31 this page described a design; the April 2026 `packages/vi` types it referred to were written against a pre-v0.1 draft and never wired, and are retired.
</Callout>
