...
description: Wire tamper-evident receipts into anthropics/commerce-agents on the Messages API, the Agent SDK, and Managed Agents, verify a session offline, and know exactly what a receipt does and does not prove.
---
...
The demo's output, from a clean ship with the released 0.30.0 CLI and the SDK from PyPI (the `1 warnings` above is the package's always-on note that the narrative fields are not signature-bound; the artifacts and Merkle root are):

```text
tool calls        intent-id result-id
  search_products        ok                   art_2e3bde5a65b7e4ed… art_a449ab98b03749ca…
  get_product_details    ok                   art_dc850843d507bc28… art_9c594ed1a4accd5f…
  add_to_cart            ok                   art_56a530661bb790df… art_3a7ac26825f5468d…
  add_to_cart            blocked:provenance   art_5eb807317d55435b… art_a58a450ef62841c9…
  checkout               ok                   art_235440f60e9061ef… art_cd10ddb427c8df94… art_941690f65293e228…
  checkout hand-off      cart signed          art_cd10ddb427c8df94…
  order placed           chained              art_e8aad1f89cb61f4e…

session           receipts=11 events=6 root_verified=True
```

The same run, recorded. The fourth call is a product id the agent never saw in a catalog read, pushed at the cart the way a poisoned listing would push it. The reference's provenance gate refuses it; the recording then decodes that receipt's signed bytes to show `"status":"blocked"` and `"gate":"provenance"` inside the signature, and `treeship package verify` proves the refused add is in the sealed tree.

![The shopping demo: a search, a product read, an add, an add the provenance gate refused, the hand-off and the chained order; the refused add's signed receipt decoded; the sealed package verifying offline with that receipt's inclusion proof passing](/blog/shopping-provenance.gif)

To verify a package someone hands you, without the ship that produced it, see [Verify a session package](/commerce/verify-a-package).
...
[Verifiable Intent](/integrations/verifiable-intent) is the Mastercard-maintained v0.1 draft for proving an agent was authorized to buy: an issuer credential, a user mandate that delegates to the agent's key with constraints, and a Layer 3 pair the agent signs for the network and the merchant. [`treeship vi attest`](/cli/vi) signs that pair with the spec's optional `agent_attestation` claim pointing at this receipt chain. Run at hand-off, the chain head it names is the hand-off receipt, so the credential binds the cart that went to checkout to the mandate that authorized it.
