...
| `receipt_body_binding` | The headline, narrative, timeline summaries and side-effect lists are under the producer's signature: the close record signs the digest of the whole `receipt.json`, so an edit fails `receipt_binding`. | That any of it is true. It is the producer's own account, signed by the producer. Quote artifacts as evidence, not the narrative. `WARN` on a package with no close record (before 0.31.4). |
...
| `receipt_binding` | The producer's close record, sealed beside the package, signs the digest of `receipt.json` and the session id. If anyone added, removed or reordered a sealed artifact after the producer closed the session, this fails, even when every `signature:` row passes. | That the producer did not re-sign a rewritten set with their own key. That is what anchoring is for. |
| `session_window` | Every sealed artifact was signed inside the session's own window. An artifact signed minutes after the session closed is the shape a spliced one has. | Wall-clock truth. The window is the signer's clock too. |
...
<Callout type="warn">
`package verify` checks that the signatures are valid for the keys the package names. It cannot decide whether those keys are the producer's; that is why, until you pin them, the verdict reads `signatures-pass` and not `verified`, and `signer_trust` stays a warning. Step 2 is not optional. On a package built before 0.31.2 the command fails at `envelopes`, and the signatures can only be checked the Step 2 way.
</Callout>
...
- **`anchoring: UNWITNESSED`** means no third party recorded that these bytes existed by a given time. The timestamps are the signer's clock, signed by the signer. If the dispute turns on *when*, ask whether the session was anchored in a transparency log, and run `treeship verify <id> --max-unwitnessed 1h` to fail unless every stretch of work was witnessed within your bound. Only a Rekor entry that verifies offline counts, at Rekor's signed time. An anchor that is only a line in the producer's local record reports as `claimed but unverified` and does not count (TS-2026-003). A package whose artifacts were anchored carries the proofs under `anchors/`, and `package verify` checks them in an `anchoring` row. JSON output carries the same judgement under `anchoring`.
...
`treeship verify <id> --full` draws the chain as a timeline. `--format json` returns every check by artifact id, `outcome`, `passed`, `failed`, `authority_failed`, `chain_linkage_ok`, `authority_ok` and `authority_checked`, for a script. An action its mandate does not authorize is a failed verification without any flag: `outcome` is `fail`, `failed` counts it, and the exit code is 1.
...
6. **Not the story.** Everything under `receipt_body_binding`: headline, narrative, timeline summaries. The producer signed them, so they cannot be edited unnoticed, but they are the producer's prose, not evidence.
