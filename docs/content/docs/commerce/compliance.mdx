...
- **Tamper-evident records.** Editing a signed artifact's *signed payload* changes its content-addressed ID and breaks verification. This is not immutability: unsigned wrapper/index fields can still change without affecting the ID, and the key holder can always sign a new artifact -- a receipt proves what was signed, not that nothing could have been re-signed.
- **Approval chains.** Nonce-bound approvals record which approver URI authorized which action. `--approver human://alice` is a URI signed with the ship's key; it proves a human identity only when that approver has their own pinned signing key and signs with it themselves, not merely by being named.
...
treeship attest action \
  ...
  --action data.process \
  --input-digest sha256:d7a8fbb307d7809469ca9abcb0082e4f8d5651e46d3cdb762d02d0bf37c9e592 \
