...
| `--structural-only` | Check structure and approvals only; do not require or verify the envelopes. For packages built before 0.31.2, which carry no envelopes and otherwise fail at `envelopes`. The verdict line says `package structure verified (signatures not checked)`. |
...
| `envelopes` | Only emitted when it fails: the package carries no `artifacts/` envelopes (built before 0.31.2) or no readable `keys.json`. `WARN` under `--structural-only` |
