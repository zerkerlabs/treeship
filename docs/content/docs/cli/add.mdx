...
- **Claude Code** — installs the official Treeship plugin (`claude plugin marketplace add zerkerlabs/treeship` then `claude plugin install treeship@treeship`) through the `claude` CLI when it is on PATH and you say yes (or `--all`); otherwise it prints those two commands. The plugin carries the hooks and the MCP server, so no config file of yours is edited. Versions before 0.31.10 wrote `~/.claude/mcp.json`, a file Claude Code does not read; `add` removes that entry if it is exactly the one it wrote.
