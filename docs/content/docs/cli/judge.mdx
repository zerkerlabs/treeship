...
<Callout type="warn">
**Next release.** `--state-out` ships on `main` but is not in the latest tagged CLI (0.31.9 rejects it). Check `treeship --version` against the [CHANGELOG](https://github.com/zerkerlabs/treeship/blob/main/CHANGELOG.md) before relying on it.
</Callout>
...
<Callout type="warn">
**Next release.** `--contract` ships on `main` but is not in the latest tagged CLI (0.31.9 rejects it). Check `treeship --version` against the [CHANGELOG](https://github.com/zerkerlabs/treeship/blob/main/CHANGELOG.md) before relying on it.
</Callout>
...
<Callout type="warn">
**Next release.** `--resolve`/`--by`/`--decision`/`--route` ship on `main` but are not in the latest tagged CLI (0.31.9 rejects them). Check `treeship --version` against the [CHANGELOG](https://github.com/zerkerlabs/treeship/blob/main/CHANGELOG.md) before relying on them.
</Callout>
...
| `--contract <ID@VERSION>` (next release) | The decision contract the judgement runs under, into every receipt |
| `--state-out <FILE>` (next release) | Write the canonical state the receipt's `state_digest` commits to |
| `--enforce` | Exit 2 on `deny` and 3 on `ask`, for shell gates |
| `--resolve <ART_ID>` (next release) | Resolve a judgement instead of judging; needs `--by` and `--decision` |
| `--by <URI>` (next release) | Who decided (`human://alice`, or an `agent://` for a stronger judge) |
| `--decision <DECISION>` (next release) | `allow`, `deny` or `route`; `route` takes `--route <name>` |
| `--reason <TEXT>` (next release; requires `--resolve`) | Why, in the decider's words |
