...
An action whose own mandate does not authorize it (out of scope, expired, wrong holder, revoked) **fails verification by default**: the top line reads `AUTHORITY INVALID`, `--format json` reports `outcome: "fail"` with the mandate verdicts counted in `failed` and named in `authority_failed`, and the exit code is 1. No flag is needed for that, and it holds in every output mode: the short output, `--format json`, and `--full`, whose verification summary carries an `authority` row. `--require-authority` is the stricter mode: it also fails when the receipt carries no mandate at all, or when a layer (typically revocation) could not be checked, because absence of a claim is not a passing check. Every failure exits 1; there is one ladder. The same holds for the two opt-in policies, `--require-authority` and `--max-unwitnessed`: when either refuses, `--format json` reports `outcome: "fail"` and names the policy in `failed_gates` (`require_authority`, `max_unwitnessed`), so a script reading only the top line never gets a green from a failed policy.
...
The human-readable page at `treeship.dev/receipt/<id>` is also accepted: the CLI maps `/receipt/<id>` to `/v1/receipt/<id>`, keeps a URL that already names the API (`/v1/receipt/<id>`, or the site's `/api/receipt/<id>` mirror) as written, never rewrites the host, drops a trailing slash and keeps the query string. A URL that names no receipt is refused, not fetched.
</Tab>
<Tab value="File path">
```bash
