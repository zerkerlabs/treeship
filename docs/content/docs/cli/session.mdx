...
4. Builds a [`.treeship` package](/docs/concepts/session-receipts#package-format) under `.treeship/sessions/<session_id>.treeship/` with `receipt.json`, `merkle.json`, `render.json`, per-artifact inclusion proofs, a signed package manifest, and a static `preview.html`.
5. Signs a `session.close` chain artifact as before.
6. Prints the package path, the canonical receipt digest, the signed manifest digest, and the Merkle root.
...
✓ session receipt composed
  ...
  manifest:  sha256:4c0f2d9d8f18e3a0b1c2d3e4f5a60718293a4b5c6d7e8f90123456789abcdef01
  signed:    art_7c75660cfc939a5bcd1a5b8a17c8f3d4
...
Upload a closed session's receipt JSON to the configured hub and print the permanent public URL.
...
3. Runs local package verification and includes the verification posture in JSON output.
4. DPoP-signs a `PUT {hub_endpoint}/v1/receipt/{session_id}` request using the active hub connection's private key.
5. Prints the permanent receipt URL.
...
<Callout type="info">
The receipt URL is permanent and public. No token, no expiry, no auth required to fetch it. It is useful for summary-level checks. Full verification still uses `treeship package verify` on the local `.treeship` package. See [`GET /v1/receipt/{session_id}`](/docs/api/receipt-get) for the API reference.
</Callout>
