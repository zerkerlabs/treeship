...
| Option | Description |
|---|---|
| `--type <TYPE>` | Event type: agent.called_tool, agent.wrote_file, agent.read_file, agent.connected_network, agent.completed_process, agent.decision (requires --model), agent.handoff, or agent.note (text in --meta) |
