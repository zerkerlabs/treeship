...
| `--structural-only` | Check structure and approvals only; do not require or verify the artifact envelopes. Needed for packages built before 0.31.2, which carry no envelopes and otherwise fail. The verdict says so |
