...
```bash
treeship grant issue --scope <scope> --audience <aud> --expiry <rfc3339 | 30d | 12h> [--parent <grant-id>]
treeship grant list
...
`--expiry` takes an RFC 3339 instant or a duration from now (`30d`, `12h`, `45m`, `90s`), which cannot fall into the past the way a fixed example date did.
