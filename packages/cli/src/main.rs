...
enum SessionCommand {
    ...
    ///
    /// Moves session.json/session.closing and the active session event
    /// directory under .treeship/quarantine-* so the next session can
    /// start cleanly while the evidence remains inspectable.
    Abandon,
...
fn dispatch(cli: &Cli, printer: &Printer) -> Result<(), Box<dyn std::error::Error>> {
    match &cli.command {
        ...
        Command::Session(sub) => match sub {
            ...
            SessionCommand::Abandon => commands::session::abandon(printer),
